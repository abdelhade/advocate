...
<template>
    ...
    <TenantLayout>
        <template #title>فاتورة اشتراك المنصة</template>
        ...
        <div class="print:hidden flex items-center justify-between gap-4 mb-8">
            <div class="flex items-center gap-2 text-xs text-stone-400 font-bold">
                <Link :href="route('tenant.billing.index')" class="hover:text-blue-700 transition">الاشتراك والفوترة</Link>
                <span>/</span>
            ...
            <div class="flex items-center gap-3">
                <button
                    @click="printInvoice"
                    class="px-5 py-2.5 bg-blue-700 hover:bg-blue-800 text-white text-xs font-bold rounded-xl shadow-md transition flex items-center gap-2"
                >
        ...
        <div class="max-w-4xl mx-auto bg-white rounded-3xl border border-stone-200 shadow-xl p-8 md:p-12 print:shadow-none print:border-none print:p-0 print:m-0 print:max-w-none text-right dir-rtl">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pb-8 border-b-2 border-stone-100">
                <div>
                    <div class="flex items-center gap-3 mb-2">
                        <div class="w-10 h-10 rounded-xl bg-blue-700 flex items-center justify-center text-white shadow-md">
                        ...
                        <div>
                            ...
                            <span class="block text-[10px] text-stone-400 font-bold uppercase">Jalsateg Legal SaaS</span>
                        </div>
                    </div>
                    <p class="text-xs text-stone-500 font-semibold">فاتورة رسمية لاشتراك واستخدام نظام المحاماة</p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 my-8 pb-8 border-b border-stone-100">
                <div class="bg-stone-50/70 rounded-2xl p-6 border border-stone-200/80">
                    <p class="text-xs font-black text-stone-400 uppercase tracking-widest mb-3">تفاصيل المفوتر إليه</p>
                    ...
                    <div class="space-y-1.5 text-xs text-stone-600 font-semibold">
                        <p><strong class="text-stone-800">صاحب المكتب / المالك:</strong> {{ invoice.owner_name }}</p>
                ...
                <div class="bg-stone-50/70 rounded-2xl p-6 border border-stone-200/80 flex flex-col justify-between">
                    <div>
                        <p class="text-xs font-black text-stone-400 uppercase tracking-widest mb-3">حالة الدفع والاستحقاق</p>
                ...
                <table class="w-full text-right border-collapse">
                    <thead>
                        <tr class="bg-stone-100 text-stone-600 text-xs font-bold border-b border-stone-200">
                            <th class="py-3.5 px-6 rounded-r-xl">الخدمة / الباقة</th>
                            <th class="py-3.5 px-6">فترة الفوترة</th>
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 pt-6 border-t border-stone-200">
                <div class="text-xs text-stone-400 space-y-1">
                    <p class="font-bold text-stone-600">ملاحظات الفاتورة:</p>
                    <p>فاتورة الكترونية رسمية موثقة صادرة عن إدارة منصة جلسات.</p>
                </div>
                ...
                <div class="w-full sm:w-72 space-y-2 text-xs font-semibold text-stone-700 bg-stone-50 p-5 rounded-2xl border border-stone-200">
                    <div class="flex justify-between">
                        <span class="text-stone-500">المبلغ الإجمالي:</span>
                    ...
                    <div class="flex justify-between pt-2 border-t border-stone-200 text-sm font-black text-stone-900">
                        <span>الصافي السديد:</span>
                        <span class="font-mono text-blue-700 text-base">{{ formatCurrency(invoice.total_amount) }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-12 pt-6 border-t border-stone-100 text-center text-[10px] text-stone-400 font-semibold">
                منصة جلسات لإدارة مكاتب المحاماة — support@jalsateg.com
            </div>
        </div>
    </TenantLayout>
</template>
