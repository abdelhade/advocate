    ...
    <div class="min-h-screen bg-stone-50 dark:bg-slate-950 text-stone-900 dark:text-slate-100 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans dir-rtl selection:bg-blue-600 selection:text-white transition-colors">
        ...
        <div class="sm:mx-auto sm:w-full sm:max-w-md text-center">
            <Link href="/" class="inline-flex items-center gap-3">
                ...
                <span class="text-3xl font-black text-stone-900 dark:text-white tracking-tight">منصة جلسات</span>
            </Link>
            <h2 class="mt-4 text-2xl font-black text-stone-900 dark:text-white tracking-tight">
                تسجيل الدخول للمكتب
            </h2>
            <p class="mt-1.5 text-xs font-bold text-stone-500 dark:text-slate-400">
        ...
        <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md px-4 sm:px-0">
            <div class="bg-white dark:bg-slate-900 py-8 px-6 shadow-xl shadow-stone-200/60 dark:shadow-slate-950/80 rounded-3xl border border-stone-200/80 dark:border-slate-800 sm:px-10">
                ...
                <!-- Status Alert -->
                <div v-if="status" class="mb-6 p-4 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 rounded-2xl text-emerald-800 dark:text-emerald-300 text-xs font-bold text-center">
                    {{ status }}
                </div>
                ...
                <form @submit.prevent="submit" class="space-y-5">
                    <!-- Email Input -->
                    <div>
                        <label for="email" class="block text-xs font-bold text-stone-700 dark:text-slate-300 mb-1.5">البريد الإلكتروني *</label>
                        <input
                            id="email"
                            type="email"
                            v-model="form.email"
                        ...
                        />
                        <p v-if="form.errors.email" class="mt-1 text-xs font-bold text-rose-600 dark:text-rose-400">{{ form.errors.email }}</p>
                    </div>

                    <!-- Password Input -->
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label for="password" class="block text-xs font-bold text-stone-700 dark:text-slate-300">كلمة المرور *</label>
                            <Link
                                v-if="canResetPassword"
                                :href="route('password.request')"
                                class="text-xs font-bold text-blue-700 dark:text-blue-400 hover:underline"
                            >
                                نسيت كلمة المرور؟
                            </Link>
                        </div>
                        <input
                            ...
                            class="w-full px-4 py-3 bg-stone-50 dark:bg-slate-800 border border-stone-200 dark:border-slate-700 rounded-xl text-stone-900 dark:text-white placeholder-stone-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 text-sm font-medium transition"
                        />
                        <p v-if="form.errors.password" class="mt-1 text-xs font-bold text-rose-600 dark:text-rose-400">{{ form.errors.password }}</p>
                    </div>
                    ...
                    <div class="flex items-center">
                        <label class="flex items-center cursor-pointer">
                            <input
                                ...
                                class="w-4 h-4 text-blue-700 rounded border-stone-300 dark:border-slate-700 focus:ring-blue-500 transition cursor-pointer"
                            />
                            <span class="ms-2.5 text-xs font-bold text-stone-600 dark:text-slate-400">تذكر تسجيل دخولي</span>
                        </label>
                    </div>
                ...
                <div class="mt-6 text-center border-t border-stone-100 dark:border-slate-800 pt-5 flex items-center justify-center gap-3 text-xs font-semibold text-stone-500 dark:text-slate-400">
                    ...
                    <Link href="/register" class="text-blue-700 dark:text-blue-400 font-bold hover:underline">إنشاء مكتب جديد</Link>
                    <span class="text-stone-300 dark:text-slate-700">•</span>
                    <Link href="/" class="text-stone-500 dark:text-slate-400 hover:text-stone-800 dark:hover:text-white transition">الرئيسية</Link>
                </div>
            </div>
        </div>
    </div>
